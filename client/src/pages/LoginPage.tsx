...
export function LoginPage({ register = false }: { register?: boolean }) {
  ...
  return <div className="auth-page"><aside className="auth-aside"><Brand light /><div className="auth-aside__copy"><span className="auth-aside__eyebrow"><Sparkles size={15} /> Plan charging with confidence</span><h1>Find and reserve your next charging stop.</h1><p>Plug-In connects public stations and community chargers in one place.</p></div><div className="auth-aside__proof"><span><CheckCircle2 size={17} /> Availability visibility</span><span><CheckCircle2 size={17} /> Simple reservations</span><span><CheckCircle2 size={17} /> Community listings</span></div></aside><main className="auth-main"><div className="auth-card"><Link className="back-link" to="/"><ArrowLeft size={16} /> Back to home</Link><div className="auth-card__title"><span className="mini-logo"><BatteryCharging size={20} /></span><h2>{register ? 'Create your account' : 'Welcome back'}</h2><p>{register ? 'Start with a demo persona, then make the network your own.' : 'Sign in to plan your next charging stop.'}</p></div><form onSubmit={(event) => { event.preventDefault(); void continueDemo() }}><label className="form-field"><span>Email address</span><input type="email" placeholder="you@example.com" defaultValue={register ? '' : 'demo@plugin.demo'} required /></label><label className="form-field"><span>{register ? 'Create a password' : 'Password'}</span><input type="password" placeholder="••••••••" defaultValue={register ? '' : 'plugin'} required /></label>{register && <label className="form-field"><span>Full name</span><input placeholder="Your name" required /></label>}<div className="demo-role-picker"><div><b>Demo persona</b><span>Choose the experience you want to explore.</span></div><div className="persona-grid">{personas.map(({ role: personaRole, label, detail, icon: Icon }) => <button key={personaRole} type="button" onClick={() => setRole(personaRole)} className={`persona-card ${role === personaRole ? 'persona-card--selected' : ''}`}><Icon size={19} /><span><b>{label}</b><small>{detail}</small></span></button>)}</div></div>{error && <p className="form-error">{error}</p>}<Button type="submit" fullWidth loading={isLoading}>{register ? 'Create demo account' : 'Continue to demo'} <ArrowRight size={17} /></Button></form><p className="auth-switch">{register ? 'Already have an account?' : 'New to Plug-In?'} <Link to={register ? '/login' : '/register'}>{register ? 'Log in' : 'Create an account'}</Link></p><p className="auth-disclaimer">Demo mode stores only a local session. Availability shown is a transparent simulation.</p></div></main></div>
}
