...
export function StationFiltersBar({ filters, onChange, count }: { filters: StationFilters; onChange: (filters: StationFilters) => void; count: number }) {
  ...
  return <div className="filter-bar">
    <label className="search-box"><Search size={18} /><input value={filters.search} onChange={(event) => onChange({ ...filters, search: event.target.value })} placeholder="Search a place or charger" /></label>
